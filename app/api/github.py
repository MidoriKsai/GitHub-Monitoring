from fastapi import APIRouter, HTTPException, Depends, WebSocket, WebSocketDisconnect
from sqlmodel import select
import httpx
from sqlalchemy.ext.asyncio import AsyncSession
from app.db.db import get_db
from app.nats.nats_events import fetch_github_data
from app.models.github_repo import GitHubRepo, GitHubRepoUpdate
from app.ws.connection_manager import GitHubWSManager
from dotenv import load_dotenv
load_dotenv()
import os

router = APIRouter(prefix="")
github_ws_manager = GitHubWSManager()
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")

# -----------------------------
# Репозитории
# -----------------------------

@router.get("/repos", response_model=list[GitHubRepo])
async def list_repos(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(GitHubRepo))
    return result.scalars().all()


@router.get("/repo/{repo_id}", response_model=GitHubRepo)
async def get_repo(repo_id: int, db: AsyncSession = Depends(get_db)):
    repo = await db.get(GitHubRepo, repo_id)
    if not repo:
        raise HTTPException(status_code=404, detail="Repository not found")
    return repo


@router.post("/create_repo")
async def create_repo(name: str, private: bool = True, db: AsyncSession = Depends(get_db)):
    url = "https://api.github.com/user/repos"
    headers = {
        "Authorization": f"token {GITHUB_TOKEN}",
        "Accept": "application/vnd.github+json"
    }
    payload = {"name": name, "private": private, "auto_init": True}

    async with httpx.AsyncClient() as client:
        response = await client.post(url, json=payload, headers=headers)

    if response.status_code != 201:
        raise HTTPException(status_code=response.status_code, detail=response.json())

    repo_data = response.json()
    repo = GitHubRepo(
        owner=repo_data["owner"]["login"],
        name=repo_data["name"],
        url=repo_data["html_url"],
        private=repo_data.get("private", False)
    )
    db.add(repo)
    await db.commit()
    await db.refresh(repo)

    await github_ws_manager.broadcast({
        "event": "repo_created",
        "repo": repo.dict()
    })

    return {"message": "Repository created successfully", "repo": repo.dict()}


@router.patch("/repo/{repo_id}", response_model=GitHubRepo)
async def patch_repo(repo_id: int, patch: GitHubRepoUpdate, db: AsyncSession = Depends(get_db)):
    repo = await db.get(GitHubRepo, repo_id)
    if not repo:
        raise HTTPException(status_code=404, detail="Repository not found")

    old_name = repo.name
    for k, v in patch.dict(exclude_unset=True).items():
        setattr(repo, k, v)

    db.add(repo)
    await db.commit()
    await db.refresh(repo)

    github_url = f"https://api.github.com/repos/{repo.owner}/{old_name}"
    headers = {
        "Authorization": f"token {GITHUB_TOKEN}",
        "Accept": "application/vnd.github+json"
    }
    github_payload = {}
    if patch.name: github_payload["name"] = patch.name
    if patch.private is not None: github_payload["private"] = patch.private

    if github_payload:
        async with httpx.AsyncClient() as client:
            response = await client.patch(github_url, json=github_payload, headers=headers)
            if response.status_code not in (200, 201):
                raise HTTPException(status_code=response.status_code, detail=response.json())

    await github_ws_manager.broadcast({
        "event": "repo_updated",
        "repo": repo.dict()
    })

    return repo


@router.delete("/repo/{repo_id}", status_code=204)
async def delete_repo(repo_id: int, db: AsyncSession = Depends(get_db)):
    repo = await db.get(GitHubRepo, repo_id)
    if not repo:
        raise HTTPException(status_code=404, detail="Repository not found")

    github_url = f"https://api.github.com/repos/{repo.owner}/{repo.name}"
    headers = {"Authorization": f"token {GITHUB_TOKEN}", "Accept": "application/vnd.github+json"}

    async with httpx.AsyncClient() as client:
        response = await client.delete(github_url, headers=headers)
        if response.status_code not in (204, 200):
            raise HTTPException(status_code=response.status_code, detail=response.json())

    await db.delete(repo)
    await db.commit()

    await github_ws_manager.broadcast({
        "event": "repo_deleted",
        "repo": {"id": repo.id, "owner": repo.owner, "name": repo.name}
    })

    return {"message": f"Repository {repo.owner}/{repo.name} deleted successfully"}


# -----------------------------
# Фоновая задача GitHub
# -----------------------------

@router.post("/tasks/run")
async def run_github_task(db: AsyncSession = Depends(get_db)):
    await fetch_github_data(db)
    return {"message": "GitHub data sync started"}


# -----------------------------
# WebSocket
# -----------------------------

@router.websocket("/ws/repos")
async def ws_repos(websocket: WebSocket):
    await websocket.accept()
    await github_ws_manager.connect(websocket)
    try:
        while True:
            data = await websocket.receive_text()
            if data.lower() == "ping":
                await websocket.send_text("pong")
    except WebSocketDisconnect:
        await github_ws_manager.disconnect(websocket)
